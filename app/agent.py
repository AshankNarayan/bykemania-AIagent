...
from app.services.data_requirement_analyzer import DataRequirementAnalyzer
from app.services.query_capability_router import QueryCapabilityRouter
...
class BykeManiaAgent:
    ...
    Behavior:
    - Groq capability router detects advanced admin tasks before normal parser
    - Advanced analytics query: asks for required data instead of hallucinating
    ...
    def __init__(self):
        ...
        self.capability_router = QueryCapabilityRouter()
        self.data_requirement_analyzer = DataRequirementAnalyzer()
    ...
    async def process_query(self, user_query: str) -> dict:
        ...
        try:
            capability = await self.capability_router.classify(user_query)

            print(f"🧭 Capability: {capability}")

            if capability.get("should_request_data"):
                data_requirement_response = (
                    self.data_requirement_analyzer.analyze_capability(
                        user_query=user_query,
                        request_id=request_id,
                ...
                if data_requirement_response:
                    print(
                        f"📊 Data requirement detected: "
                        f"{data_requirement_response.get('task_type')}"
                    )

                    self._safe_log(
                        "finish_success",
                        request_id=request_id,
                        formatted_response=data_requirement_response,
                    )

                    return data_requirement_response

            fallback_data_requirement_response = self.data_requirement_analyzer.analyze(
                user_query=user_query,
                request_id=request_id,
            )

            if fallback_data_requirement_response:
                print(
                    f"📊 Fallback data requirement detected: "
                    f"{fallback_data_requirement_response.get('task_type')}"
                )

                self._safe_log(
                    "finish_success",
                    request_id=request_id,
                    formatted_response=fallback_data_requirement_response,
                )

                return fallback_data_requirement_response

            query_intent = await self.parser.parse(user_query)
            parsed_intent = query_intent.model_dump()

            parsed_intent["capability"] = capability
            ...
            response_data["capability"] = capability
